...
import {
  carStats, fmtSana, kunFarqi, parseItemInput, qaytishKuni, tashkentDate,
...
export default function RasxodCarCard({
  car, now, highlighted, onEdit, onDelete, onAddItem, onUpdateItem, onDeleteItem, onSetTulov,
}: Props) {
    ...
    >
      ...
      <div style={{ borderTop: '1px solid var(--border)', background: 'rgba(0,0,0,0.14)' }}>
        {car.items.map((item) => {
          ...
          const days = item.tulandi ? qaytishKuni(item) : kunFarqi(item.sana, now);
          return (
            ...
            <div key={item.id} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px 16px', padding: '10px 20px', borderBottom: '1px solid var(--border)' }}>
              <div style={{ flex: '1 1 300px', minWidth: 0, display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <span style={{ flex: '1 1 auto', minWidth: 0, fontSize: 13, fontWeight: 600, color: item.tulandi ? 'var(--text2)' : 'white', overflowWrap: 'anywhere' }}>
                  {item.nom}
                </span>
                <span style={{ flex: '0 0 auto', textAlign: 'right', fontSize: 13.5, fontWeight: 800, color: item.tulandi ? 'var(--text3)' : 'white', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
                  {fmt(item.summa)}
                </span>
              </div>
                ...
                {item.tulandi ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      title="Mijoz shu rasxod pulini qaytardi"
                    >
                      {payBusy ? spinner : <Check size={14} />} {"To'landi"}
                    </button>
                  </>
                )}
