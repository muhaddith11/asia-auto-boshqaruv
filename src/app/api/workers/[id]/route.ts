...
const WORKER_COLUMNS = 'id, ism, familiya, tel, mutax, foiz, status, role, bolim, telegram, login, parol, "shareType", "parentId", created_at';
...
function mapAppToDB(body: any) {
  const b = { ...body } as any;
  const allowed = ['id', 'ism', 'familiya', 'tel', 'mutax', 'foiz', 'status', 'role', 'bolim', 'telegram', 'login', 'parol', 'shareType', 'parentId'];
  const clean: any = {};
  allowed.forEach(key => {
