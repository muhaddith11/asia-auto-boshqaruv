...
import { X, Plus, Package, CheckCircle2, DollarSign, Settings } from 'lucide-react';
import { formatDigits, stripToDigits } from '@/lib/numberInput';

export default function AddPartPage() {
  const router = useRouter();
  const { addZapchast, mashinalar } = useStore();
  ...
  return (
    <div className="flex-1 flex items-center justify-center bg-[#0d1220]/60 backdrop-blur-[6px] min-h-screen p-6">
      <div className="bg-[#1a1c24] border border-[#2a2d3d] rounded-2xl shadow-2xl w-full max-w-[520px] overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <div className="px-6 py-4 border-b border-[#2a2d3d] flex items-center justify-between bg-[#1a1c24]">
          <h3 className="font-bold text-[14px] text-white uppercase tracking-wider flex items-center gap-2">
             <Package size={18} className="text-indigo-500" /> Zapchast qo'shish
          </h3>
        ...
        <form onSubmit={handleSubmit} className="p-7 space-y-5">
          ...
          <div className="space-y-1">
            <label className="block text-[13px] font-bold text-slate-500 mb-1 pl-1">Zapchast nomi *</label>
            <div className="relative group">
              <Package size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-indigo-400 transition-colors" />
              <input 
          ...
          <div className="space-y-1">
            <label className="block text-[13px] font-bold text-slate-500 mb-1 pl-1">Mashina turi</label>
            <div className="relative group">
              <Settings size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-amber-400 transition-colors" />
              <select 
          ...
          <div className="grid grid-cols-2 gap-4">
             <div className="space-y-1">
                <label className="block text-[13px] font-bold text-slate-500 mb-1 pl-1">Kelish narxi *</label>
                <div className="relative group">
                  <DollarSign size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-red-400 transition-colors" />
                  <input
                    type="text" inputMode="numeric" required value={formatDigits(formData.sebestoimost)}
                    onChange={e => setFormData({...formData, sebestoimost: stripToDigits(e.target.value)})}
                    className="w-full bg-[#1e212b] border border-[#2a2d3d] rounded-xl pl-12 pr-4 py-4 outline-none focus:border-red-500/50 text-red-400 text-[15px] font-bold transition-all"
                    placeholder="0"
                  />
                </div>
             </div>
             <div className="space-y-1">
                <label className="block text-[13px] font-bold text-slate-500 mb-1 pl-1">Sotish narxi *</label>
                <div className="relative group">
